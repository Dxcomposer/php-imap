...
namespace Dxkjcomposer\PhpImap;
...
use Dxkjcomposer\PhpImap\Exceptions\ConnectionException;
use Dxkjcomposer\PhpImap\Exceptions\InvalidParameterException;
use const SA_ALL;
use const SE_FREE;
use const SE_UID;
...
class Mailbox
{
    ...
    /**
     * format header raw to array
     * @param $headerRaw
     * @return array
     */
    protected function headerFormatArrays($headerRaw): array
    {
        $headerArr = explode("\r\n", $headerRaw);
        $arr = [];
        $preKey = '';
    ...
    protected function initMailPart(IncomingMail $mail, object $partStructure, $partNum, bool $markAsSeen = true, bool $emlParse = false): void
    {
        ...
        $headersArr = $this->headerFormatArrays($mail->headersRaw);
        ...
        if ('RFC822' === $partStructure->subtype && isset($partStructure->disposition) && 'attachment' === $partStructure->disposition) {
            ...
            if (!isset($headersArr['ClientAttach'])) {
